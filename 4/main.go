...
var RoomRegex = regexp.MustCompile(`([a-z]+(?:-[a-z]+)*)-(\d+)\[([a-z]{5})\]`)

func ParseLine(line string) *Room {
	matches := RoomRegex.FindAllStringSubmatch(line, -1)
	if len(matches) != 1 || len(matches[0]) != 4 {
		log.Fatalf("Got invalid match result: %#v", matches)
	}

	id, err := strconv.Atoi(matches[0][2])
	if err != nil {
		log.Fatalf("Failed to parse room id %s from %s", matches[0][2], line)
	}
...
func ReadRooms() []*Room {
	...
	for s.Scan() {
		rooms = append(rooms, ParseLine(s.Text()))
	}
...
var _ sort.Interface = (*CountList)(nil)
...
func (r *Room) IsValid() bool {
	...
	for l, c := range tally {
		counts = append(counts, &Count{L: l, C: c})
	}

	sort.Sort(counts)
	cksum := []byte{}
	for i := 0; i < 5; i += 1 {
		cksum = append(cksum, counts[i].L)
	}

	log.Print(*r)
	log.Print(tally)
	log.Print(string(cksum))
	return strings.Compare(string(cksum), r.Checksum) == 0
}
...
// input at the bottomb because it's a paint in the ass
